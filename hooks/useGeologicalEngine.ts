...
import { createRulesEngine, ALL_INSTRUCTION_BLOCKS } from '@/modules/geological';  
import type { 
  GeologicalInput, 
  RulesEngineResult 
} from '@/modules/geological/types';
...
interface GenerationResult {
  success: boolean;
  technicalAssignment?: RulesEngineResult;
  error?: string;
...
export function useGeologicalEngine() {
  ...
  const generateTechnicalAssignment = useCallback(
    async (customInput?: Partial<GeologicalInput>): Promise<GenerationResult> => {
      ...
      try {
        ...
        // Создаём движок
        const engine = createRulesEngine(ALL_INSTRUCTION_BLOCKS, {
          verboseLogging: true,
          autoSelectVariant: true
        });
        
        // Генерируем ТЗ - ИСПРАВЛЕННАЯ СТРОКА
        const technicalAssignment = await engine.process(input);
        ...
        console.log(`🔨 Всего работ: ${technicalAssignment.allWorks.length}`);
  ...
  const getStatistics = useCallback(() => {
    ...
    return {
      totalBlocks: ta.statistics.totalBlocksEvaluated,
      appliedBlocks: ta.statistics.blocksApplied,
      totalWorks: ta.allWorks.length,
      mandatoryWorks: ta.statistics.mandatoryWorks,
      recommendedWorks: ta.statistics.recommendedWorks,
      warnings: ta.warnings.length,
      conflicts: ta.conflicts.length
    };
  ...
  const getWorksByModule = useCallback((module: string) => {
    ...
    return ta.allWorks.filter(work => work.module === module);
  }, [store.technicalAssignment]);
  ...
  const getWorksByCategory = useCallback((category: 'mandatory' | 'recommended' | 'optional') => {
    ...
    return ta.allWorks.filter(work => work.category === category);
  }, [store.technicalAssignment]);
